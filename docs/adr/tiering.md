...
| `tests/backtests/`                                                                      | 2 — Validation harness             | Standard 1-reviewer PR. Read-only: calls but never modifies `risk_engine/`. Because it validates Tier 0/1 logic, apply Tier 0/1 merge discipline — do not merge in the same session the code was written; re-read the diff as if reviewing someone else's work before merging. |
| `tests/integration/`, `tests/dashboard/`                                                | 2 — Integration & contract tests   | Standard 1-reviewer PR. Read-only validation: verifies integration and contract consistency between services while executing Tier 0/1 risk-engine code in-process, without modifying any risk-engine logic.                                                                    |
